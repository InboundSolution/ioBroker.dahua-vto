 ...
 *
 * Jede Anfrage trägt die Session-ID im binären Header UND als "session"-Feld
 * im JSON-Body (so auch im Vorbild myhomeiot/DahuaVTO; Firmware prüft das
 * Feld teils im Body und antwortet sonst mit "Invalid Request!").
 *
...
class DahuaClient extends EventEmitter {
    ...
    _handleLoginResponse(message) {
        if (this.loginPhase === 1) {
            const { random, realm, random2, encryption } = message.params || {};
            ...
            this.log.debug &&
                this.log.debug(
                    `Login-Challenge: realm=${realm}, encryption=${encryption || 'unbekannt'}, random=${random}` +
                        (random2 ? `, random2=${random2}` : ', kein random2'),
                );
    ...
    _send(message) {
        this.requestId += 1;
        // session gehört neben dem binären Header auch in den JSON-Body
        const frame = buildFrame(this.sessionId, this.requestId, { ...message, session: this.sessionId });
        if (this.socket) {
